...
export function Hero({ featuredCase, casesList }: HeroProps) {
  ...
  return (
    <section
      id="section-hero"
      className="relative w-full max-w-full min-h-[580px] sm:min-h-[660px] md:min-h-[85vh] lg:min-h-screen bg-[#0E1016] flex flex-col justify-end pb-6 sm:pb-10 md:pb-14 pt-40 sm:pt-52 md:pt-28 overflow-hidden md:snap-start"
    >
      ...
      <div className="absolute inset-0 z-0 bg-[#0E1016]">
        {heroImg && !hasHeroError && (
          <Image
            ...
            fill
            unoptimized={typeof heroImg === 'string' && (heroImg.startsWith('data:') || heroImg.startsWith('blob:') || heroImg.startsWith('http'))}
            priority
            sizes="100vw"
            onError={() => setHasHeroError(true)}
            className="object-cover object-top sm:object-center transform scale-100 sm:scale-105 transition-all duration-1000 brightness-70 sm:brightness-75"
          />
        )}
        ...
        <div className="md:hidden absolute inset-0 bg-gradient-to-t from-[#0E1016] via-[#0E1016]/65 to-transparent pointer-events-none" />
        <div className="md:hidden absolute inset-x-0 top-0 h-24 bg-gradient-to-b from-[#0E1016]/85 to-transparent pointer-events-none" />
        <div className="absolute inset-x-0 bottom-0 h-44 sm:h-48 bg-gradient-to-t from-[#0E1016] via-[#0E1016]/85 to-transparent pointer-events-none" />
      </div>

      {/* Hero Content Body */}
      <div className="relative z-10 w-full max-w-7xl mx-auto px-4 sm:px-6 md:px-12 flex flex-col justify-end">
        <div className="max-w-2xl animate-fadeIn space-y-2.5 sm:space-y-3.5">
          ...
          <div className="flex flex-wrap items-center gap-2 sm:gap-2.5">
            ...
            <span className="text-[#D4AF37] text-[10px] sm:text-xs font-mono font-semibold uppercase tracking-wider">
              {activeCase.court} · {activeCase.year}
            </span>
          </div>

          {/* Bold Billboard Title in Netflix Anton Style */}
          <h1 className="font-anton text-3xl sm:text-5xl md:text-6xl lg:text-7xl text-white uppercase tracking-tight leading-[0.95] drop-shadow-xl break-words">
            {displayTitle}
          </h1>

          {/* Subtitle / Citation in Amber Gold */}
          <p className="text-[11px] sm:text-sm font-semibold text-[#D4AF37] uppercase tracking-wider font-mono">
            {activeCase.court} · {activeCase.doctrines?.[0] || 'IPC 79'}
          </p>

          {/* Dramatic Story Hook */}
          <p className="text-xs sm:text-sm md:text-base text-[#e5e5e5] leading-relaxed line-clamp-2 sm:line-clamp-3 font-sans drop-shadow-md">
            {displayHook}
          </p>
          ...
          <div className="flex items-center gap-2 sm:gap-3 pt-1 sm:pt-2">
            ...
            <Link
              ...
              className="flex-1 sm:flex-initial flex items-center justify-center gap-1.5 sm:gap-2 px-4 sm:px-7 py-2.5 sm:py-3 bg-white hover:bg-white/90 text-[#141414] font-bold text-xs sm:text-sm uppercase tracking-wider rounded-xs transition-all shadow-xl hover:scale-105 cursor-pointer"
            >
            ...
            <Link
              ...
              className="flex-1 sm:flex-initial flex items-center justify-center gap-1.5 px-3 sm:px-6 py-2.5 sm:py-3 bg-white/20 hover:bg-white/30 text-white font-bold text-xs sm:text-sm uppercase tracking-wider rounded-xs backdrop-blur-md transition-all cursor-pointer border border-white/20"
            >
            ...
            <button
              ...
              className={`p-2.5 sm:p-3 rounded-xs border transition-all cursor-pointer text-xs sm:text-sm font-bold flex items-center justify-center min-w-[40px] sm:min-w-[44px] min-h-[40px] sm:min-h-[44px] flex-shrink-0 ${
                bookmarked
        ...
        <div className="pt-3 sm:pt-6 mt-3 sm:mt-8 border-t border-white/10 flex items-center gap-2 overflow-x-auto scrollbar-none py-1 w-full max-w-full">
